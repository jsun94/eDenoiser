...
void get_submodule_de_regnet(torch::jit::script::Module module,Net &net){
    Layer t_layer;
    Dummy residual;
...
void *predict_warm_de_regnet(Net *regnet){
	{
		...
		for(i=0;i<regnet->layers.size();i++){
			...
			thpool_add_work(thpool,(void(*)(void *))forward_de_regnet,&th);
...
void *predict_de_regnet(Net *regnet){
	{
		at::cuda::CUDAGuard guard({at::kCUDA,GPU_NUM});
		int i;
		int j;
		...
		cudaEventCreate(&start);
		cudaEventCreate(&end);
		std::vector<torch::jit::IValue> origin_input = regnet->input;
		cudaEventRecord(start);
		for(j=0;j<total_iter; j++){
			regnet->input = origin_input;
		for(i=0;i<regnet->layers.size();i++){
			pthread_mutex_lock(&mutex_t[regnet->index_n]);
			cond_i[regnet->index_n] = 1;
